...
def post_comment_like():
    ...
    try:
        commnts = post['comments']
        if ("num_likes" not in commnts[ordinal]):
           numLikes = 1
        else:
           numLikes = commnts[ordinal]["num_likes"]+1
        commnts[ordinal]["num_likes"] = numLikes
        posts.update({'permalink':permalink}, {'$set':{'comments':commnts}})
